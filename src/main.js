...
var $imgAreaEl = $('#img-area-self'),//图片放置元素
    ...
    labelIdFinish = [],
    exampleImg = ['src/dog.png', 'src/young.png'];
...
function getImage() {
    ...
    adaptionImg(exampleImg[0], $('#img-self'));
}

function getTotal(){
    return exampleImg.length;
}

//前一张照片
function previousImage(i){
    if (i = 0){
        console.log("已经是第一张照片......");
    }
    else{
        // clearAll();
        i--;
        adaptionImg(exampleImg[i], $('#img-self'));
    }
}
...
function nextImage(i){
    ...
    // var imageID = i; // 第i张照片
    // Node1 = [[labelList[i].x,labelList[i].y]]; //第一个点
    // Node2 = [[labelList[i].x+labelList[i].w,labelList[i].y+labelList[i].h]]; // 第二个点
    // var description = labelList[i].tag; // 标签内容
    // console.log(labelList.length);

    if(i<exampleImg.length-1){
        clearAll();
        i++;
        adaptionImg(exampleImg[i], $('#img-self'));
    }
    else if( i = exampleImg.length - 1){
        console.log("照片读取结束");
    }
...
//读取照片上的label
function readLabel(imageID){
    $.ajax({
        type:"GET",
        url:"/workplace/part/read",
        data: {
            imageID :imageID,
        },
...
//保存照片上的label
function saveLabel(imageID, x1, x2, y1, y2, description){
    $.ajax({
        type:"POST",
